...
class ImportarSimulacaoView(View):
    ...
    def get(self, request):
        ...
        return render(
            request,
            self.template_name,
            {"form1": form1, "clientes": clientes, "simulacao": simulacao},
        )
...
def faturamento_mensal(request):
    ...
    quantidade_kw_total = Conta.objects.all().aggregate(Sum("quantidade_kw"))[
        "quantidade_kw__sum"
    ]
    valor_unit_total = Conta.objects.all().aggregate(Sum("valor_unit"))[
        "valor_unit__sum"
    ]
    total_demanda = Conta.objects.filter(descricao__icontains="DEMANDA").aggregate(
        total=Sum("valor_unit")
    )
    demanda = total_demanda["total"] if total_demanda["total"] is not None else 0
    total_consumo = Conta.objects.filter(descricao__icontains="CONSUMO").aggregate(
        total=Sum("valor_unit")
    )
...
def medicao_fat(request):
    ...
    dados = {"clientes": clientes, "medicao": medicao}
...
def simulacao(request):
    ...
    quantidade_kw_total = Simulacao.objects.all().aggregate(Sum("quantidade_kw"))[
        "quantidade_kw__sum"
    ]
    quantidade_kw_total = round(quantidade_kw_total, 2)
    unit_c_trib_total = Simulacao.objects.all().aggregate(Sum("unit_c_trib"))[
        "unit_c_trib__sum"
    ]
...
def gerar_pdf(request):
    ...
    response = FileResponse(
        open(pdf_path, "rb"), filename=file_name, content_type="application/pdf"
    )
...
def capturar_imagem(request):
    ...
    for url in urls:
        ...
        screenshot_path = (
            f"media/capturas/{url_name}_screenshot.png"  # Define o caminho da imagem
        )
