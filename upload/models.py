...
class Fatura(models.Model):
    cliente = models.ForeignKey(
        Cliente, on_delete=models.CASCADE, related_name="faturas"
    )
    ...
    valor_demanda_max = models.DecimalField(
        max_digits=15, decimal_places=4, blank=True, null=True
    )
...
class Simulacao(models.Model):
    cliente = models.ForeignKey(
        Cliente, on_delete=models.CASCADE, related_name="simulacao"
    )
...
class DadosFatura(models.Model):
    cliente = models.ForeignKey(
        Cliente, on_delete=models.CASCADE, related_name="datafat"
    )
...
class DadosMedicao(models.Model):
    cliente = models.ForeignKey(
        Cliente, on_delete=models.CASCADE, related_name="datamed"
    )
...
class DadosEconomia(models.Model):
    cliente = models.ForeignKey(
        Cliente, on_delete=models.CASCADE, related_name="dataecon"
    )
    ...
    def livre(self):
        return (
            self.cativo_fat + self.enel_tranding
            if self.enel_tranding
            else self.cativo_fat
        )
